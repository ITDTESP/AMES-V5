    ...
    private void parseStorageInputData(CaseFileData testConf) throws BadDataFileFormatException {
        ...
        for (String s : storageData) {
            try {
                //Split the line at white space.
                String[] lineElems = s.split(WS_REG_EX);
                testConf.putStorageData(numRecog.stoi(lineElems[0]),//ID
                        numRecog.stoi(lineElems[1]),//atBus
                        numRecog.stod(lineElems[2]),//EndPointSoc
                        numRecog.stod(lineElems[3]),//MaximumEnergy
                        numRecog.stod(lineElems[4]),//NominalRampDownInput
                        numRecog.stod(lineElems[5]),//NominalRampUpInput
                        numRecog.stod(lineElems[6]),//NominalRampDownOutput
                        numRecog.stod(lineElems[7]),//NominalRampUpOutput
                        numRecog.stod(lineElems[8]),//MaximumPowerInput
                        numRecog.stod(lineElems[9]),//MinimumPowerInput
                        numRecog.stod(lineElems[10]),//MaximumPowerOutput
                        numRecog.stod(lineElems[11]),//MinimumPowerOutput
                        numRecog.stod(lineElems[12]),//MinimumSoc
                        numRecog.stod(lineElems[13])//EfficiencyEnergy
                );
            } catch (NumberFormatException nfe) {
                throw new BadDataFileFormatException(inputReader.sourceFile,
                        inputReader.lineNum,
