...
# def compute_hot_start_rule(m, g, t ):
#     if t <= value(m.ScaledColdStartTime[g]):
#         if t  <= value(m.UnitOnT0State[g]) + value(m.ScaledColdStartTime[g]):
#             m.HotStart[g, t] = 1
#             return m.HotStart[g, t]
            
#         elif t  > value(m.UnitOnT0State[g]) + value(m.ScaledColdStartTime[g]):
#              return m.HotStart[g, t] <= sum( m.UnitOn[g, i] for i in range(1, t))
...
def constraint_reserves(model, PriceSenLoadFlag=False,
    ...
    if has_global_reserves is True:
        # click.echo('global reserve constraint is disabled')
        ...
        # click.echo("global reserve up requirment >>>>>>>>>>>>>>>>>>>>>>>>>>>>>")
        # model.EnforceReserveUpRequirements.pprint()
        # model.EnforceReserveDownRequirements.pprint()
...
def constraint_for_cost(model):
    ...
    # click.echo("hot start constraintttttttttttttttttttttttttttttttt")
    # click.echo(model.ComputeHotStart.pprint())
...
# added constraints 
# def up(model,t):
#     return sum(model.R[g,t] for g in model.Generators) >= 2.5
#     # return sum(model.R[g,t] for g in model.Generators) >= model.ReserveUpRequirement[t]

# # def reserve_up_constraint(model):
# #     model.RUR = Constraint(model.TimePeriods, rule = up)
# #     model.RUR.pprint()
    

# def down(model,t):
#     return sum(model.R[g,t] for g in model.Generators) >= model.ReserveDownRequirement[t]

# def reserve_down_constraint(model):
#     model.RDR = Constraint(model.TimePeriods, rule = down)
#     model.RDR.pprint()
    

# def P_plus_R(model,t,g):
#     return (model.PowerGenerated[g,t] + model.R[g,t] <= model.MaximumPowerAvailable[g,t] )

# def P_plus_R_constraint(model):
#     model.c2 = Constraint(model.TimePeriods,model.Generators, rule = P_plus_R)
#     model.c2.pprint()
#     # for t in model.TimePeriods:
#     #     click.echo(f"reserve up requirement {value(model.ReserveUpRequirement[t])}")

# def const3(model,t,g):
#     return (model.R[g,t] <= model.Rmax[g])

# def reserve_capability(model):
#     model.c3 = Constraint(model.TimePeriods, model.Generators, rule = const3)
#     model.c3.pprint()
