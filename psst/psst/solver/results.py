...
class PSSTResults(object):
    ...
    @property
    def reserve_down_constraint_dual(self):
        return self._get('RDR', self._model, dual=True)
    
    @property
    def reserve_up_constraint_dual(self):
        return self._get('RUR', self._model, dual=True)
    ...
    def _get(attribute, model, set1=None, set2=None, dual=False):
        ...
        if set1 is not None and set2 is None:
            ...
            if set1 is None and set2 is None:
                ...
                elif attribute is 'RUR':
                    index = getattr(model, attribute)
                    _dict = list()
                    for i in index:
                        _dict.append(model.dual.get(getattr(model,attribute)[i]))
                    return pd.DataFrame(_dict)
                elif attribute is 'RDR':
                    index = getattr(model, attribute)
                    _dict = list()
                    for i in index:
                        _dict.append(model.dual.get(getattr(model,attribute)[i]))
            ...
            # click.echo(pd_unsorted)
            # click.echo(pd_unsorted.columns)
            # click.echo("----sorted-------------")
            
            # click.echo(sorted(pd_unsorted.columns))

            pd_sorted = pd_unsorted.reindex(sorted(pd_unsorted.columns),axis=1)
            
            # click.echo("lasttttttttttttt")
            # click.echo(pd_sorted)
            return pd_sorted 
